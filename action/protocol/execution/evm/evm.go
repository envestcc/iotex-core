...
func ExecuteContract(
	...
	fmt.Printf("receipt status: %d, retLen: %d, SetRevertMessageFeature: %d\n", receipt.Status, len(retval), ps.featureCtx.SetRevertMessageToReceipt)
	if ps.featureCtx.SetRevertMessageToReceipt && receipt.Status == uint64(iotextypes.ReceiptStatus_ErrExecutionReverted) && retval != nil && len(retval) >= 4 && bytes.Equal(retval[:4], _revertSelector) {
