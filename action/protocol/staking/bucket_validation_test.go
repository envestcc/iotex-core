...
func TestValidateBucket(t *testing.T) {
	...
	t.Run("validate bucket self staked", func(t *testing.T) {
		...
		r.Nil(validateBucketSelfStake(protocol.MustGetFeatureCtx(ctx), csm, bkt, false))
		r.ErrorContains(validateBucketSelfStake(protocol.MustGetFeatureCtx(ctx), csm, bkt, true), "bucket is not self staking")
		// selfstaked bucket
		...
		r.Nil(validateBucketSelfStake(protocol.MustGetFeatureCtx(ctx), csm, bkt, true))
		r.ErrorContains(validateBucketSelfStake(protocol.MustGetFeatureCtx(ctx), csm, bkt, false), "self staking bucket cannot be processed")
	})
